...
public class ControllerRB : MonoBehaviour 
{
	...
	private     int              size              ; //количество ячеек
	private     float            sizeButton        ; //размер кнопки
	private     float            RectButton        ; //координаты начала ряда
	private     bool             iController       ; //проверка на нажатие
...
size              = lenght*lenght                              ;	
sizeButton        = (Screen.height-40)/(lenght+1)              ;
float l=0.0f;
l=(float)lenght;
...
GUI.depth = 5;
GUI.DrawTexture(new Rect(RectButton,20,sizeButton*lenght,sizeButton),gameEfect[0]);

for(int g=0;g<lenght;g++)
{
if(moreCell != g+1)
			{
if(GUI.RepeatButton(new Rect(RectButton+(sizeButton*g),20,sizeButton,sizeButton),gameEfect[1]))
{
if(Time.timeScale > 0)
{
...
if(GUI.RepeatButton(new Rect(RectButton+((moreCell-1)*sizeButton),20,sizeButton,sizeButton),texture[0])) //Дополнительная ячейка
{
if(iController==true)
{
...
if(GUI.RepeatButton(new Rect(RectButton+(sizeButton*s),20+sizeButton*(l+1),sizeButton,sizeButton),texture[i]))
{
if(iController==true)
				{
coordinates[0]=s+1;
